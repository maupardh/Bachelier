...
def refresh():
    ...
    today = datetime.date(2015, 10, 31)
    ...
    asian_market_close = local_tz.normalize(my_markets.EQUITY_MARKETS_BY_COUNTRY_CONFIG['CH']['TimeZone']
                                            .localize(datetime.datetime(today.year, today.month, today.day)).astimezone(local_tz)) + \
                         my_markets.EQUITY_MARKETS_BY_COUNTRY_CONFIG['CH']['MarketClose']

    time_to_sleep_until_asia = max(asian_market_close + datetime.timedelta(minutes=30) - local_tz.localize(datetime.datetime.now().astimezone(local_tz)),
    ...
    # European Equities
    emea_market_close = local_tz.localize(
        datetime.datetime(today.year, today.month, today.day,
                          tzinfo=my_markets.EQUITY_MARKETS_BY_COUNTRY_CONFIG['GR']['TimeZone']) +
    ...
    us_market_close = local_tz.localize(
        datetime.datetime(today.year, today.month, today.day,
                          tzinfo=my_markets.EQUITY_MARKETS_BY_COUNTRY_CONFIG['US']['TimeZone']) +
...
def refresh_na(date):
    ...
    try:
        ...
        us_assets = us_assets[map(lambda country: country in my_markets.EQUITY_FEED_SOURCES_BY_COUNTRY['US'],
