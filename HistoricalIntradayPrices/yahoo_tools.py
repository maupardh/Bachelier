...
def _prepare_equity_assets(equity_assets_df):
    ...
    equity_assets_df['MNEMO_AND_FEED_SOURCE'] = zip(equity_assets_df['ID_BB_SEC_NUM_DES'], equity_assets_df['FEED_SOURCE'])
    equity_assets_df = equity_assets_df.groupby(['COMPOSITE_ID_BB_GLOBAL']).agg({'MNEMO_AND_FEED_SOURCE': lambda x: set(x),
                                                                                 'FEED_SOURCE': lambda x: set(x)})
    equity_assets_df = equity_assets_df[equity_assets_df['FEED_SOURCE']
        .apply(lambda sources: any(source in __MAP_BBG_FEED_SOURCE_TO_YAHOO_FEED_SOURCE for source in sources))]
    equity_assets_df['COUNTRY'] = equity_assets_df['MNEMO_AND_FEED_SOURCE'] \
